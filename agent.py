...
async def post_transcript(data: TranscriptRequest):
    try:
        ...
        requestData = {"transcript": formattedData}
        ...
        raise HTTPException(status_code=500, detail=f"Error during request: {str(e)}")
...
async def get_transcript():
    try:
        ...
        raise HTTPException(
            status_code=e.status, detail=f"Error fetching transcript: {str(e)}"
        )
    except Exception as e:
        raise HTTPException(
...
async def create_story(
    ...
    transcript_data = next(
        (
            ...
            if item["userRoomId"] == user_room_id and item["chapterId"] == chapter_id
        ),
        None,
    )
...
async def run_multimodal_agent(ctx: JobContext, participant: rtc.RemoteParticipant):
    ...
    )  # Ensure this returns a dict with a string key, e.g., "transcript"
    if not response.get("response", {}):
        logger.error("No transcript found in response")
        return

    logger.info(
        f"response: {response.get('response', {}).get('prompt',{}).get('generated_prompt_text', '')}"
    )
    generatedPromptText = (
        response.get("response", {}).get("prompt", {}).get("generated_prompt_text", "")
    )
